...
- Theme switcher: `Super+Shift+t`. It uses `~/.config/sway/theme-menu.sh`, stores the selected theme in `~/.config/sway/theme-current`, and applies it through `~/.config/sway/theme-apply.sh` so the choice survives Sway reloads. External GNOME Appearance/LXAppearance launcher entries were removed after GNOME/Snap-related install errors appeared under Sway; the menu now only applies installed local themes.
- Optional richer theme packages were requested but not installed by Codex because sudo could not prompt for a password. Run from a real terminal: `sudo apt install lxappearance qt5ct qt6ct arc-theme papirus-icon-theme materia-gtk-theme`.
...
- Wofi app launcher is enlarged for the 5 inch display: `width=88%`, `height=82%`, `font-size=18px`, and app icons `image_size=46`. Search is explicitly case-insensitive (`insensitive=true`) and now uses predictable `matching=contains` instead of fuzzy matching, with alphabetical sorting.
...
- Theme switcher is available at `Super+Shift+t`. It uses installed Yaru/Adwaita themes and auto-adds Arc, Materia, Papirus, and Numix options if those packages are installed. It does not launch GNOME Appearance or LXAppearance to avoid Snap-related errors under Sway.
